...
def run_training():
    ...
    mask_dir = "D:\\myPythonProjects\\ImageDenoising\\masks"
    ...
    train_loss = loss_func(output, image_batch, mask_reverse)
    ...
    with tf.Session() as sess:
        ...
        try:
            for step in np.arange(max_step):
                ...
                if (step+1) % 300 == 0:
                    save_path = saver.save(sess, "D:\\myPythonProjects\\ModelParameters\\model_reconstruction_mu35_ptg20_Unet_DiffScan.ckpt")
                    print("Save to path:", save_path)
                    # mask_temp, mask_reverse_temp = sess.run([mask, mask_reverse])
