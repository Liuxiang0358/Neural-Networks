...
def run_training(width, height, batch_size, mupathLen, SamplingRatio, learning_rate):
    train_data_dir = "D:\\myPythonProjects\\ImageDenoising\\CatVsDog"
    image_list = read_files(train_data_dir, n_data_set)

    x = tf.placeholder(dtype=tf.float32, shape=[Batch_size, Width, Height, 1])
    y = tf.placeholder(dtype=tf.float32, shape=[Batch_size, Width, Height, 1])
    mask = tf.placeholder(dtype=tf.float32, shape=[Width, Height])

    output = CNN_U_net_deep4(x, batch_size, width, height)
    train_loss = loss_func(output, y, mask)
    ...
    with tf.Session() as sess:
        ...
        try:
            for step in np.arange(max_step):
                ...
                # image = mpimg.imread(image_list[step])
                # image = np.sum(image, axis=2)/3
                # image = image.reshape(width, height)

                image_content = tf.read_file(tf.cast(image_list[step],tf.string))
                image = tf.image.decode_jpeg(image_content, channels=3)
                image = tf.image.rgb_to_grayscale(image)
                image = tf.image.resize_image_with_crop_or_pad(image, target_width=width, target_height=height)
                y_data = tf.cast(image, tf.float32)
                y_data = tf.reshape(y_data, [batch_size, width, height, 1])

                y_data = sess.run(y_data)


                mask_data = muPathMaskGen(mupathLen, Width, Height, SamplingRatio)
                mask_data_reverse = (mask_data == 0).astype(float)


                mask_data = np.reshape(mask_data, [batch_size, width, height, 1])
                x_data = np.multiply(y_data, mask_data)

                _, tra_loss = sess.run([train_optimizer, train_loss], feed_dict={x:x_data, y:y_data, mask:mask_data_reverse})

                if step % 20 == 0:
                    print(step, tra_loss)

                if (step+1) % 300 == 0:
                    save_path = saver.save(sess, "D:\\myPythonProjects\\ModelParameters\\model_reconstruction_mu35_ptg20_Unet_diffMasks.ckpt")
                    print("Save to path:", save_path)

                    recovered_image = sess.run(output, feed_dict={x:x_data})
                    f, axarr = plt.subplots(1, 3)
                    axarr[0].imshow(np.multiply(y_data[0, :, :, 0], mask_data[0, :, :, 0]), cmap='gray')
                    axarr[0].set_title("undersampled image")
                    axarr[1].imshow(recovered_image[0, :, :, 0], cmap='gray')
                    axarr[1].set_title("recovered image")
                    axarr[2].imshow(y_data[0, :, :, 0], cmap='gray')
...
run_training(Width, Height, Batch_size, mupathLen, SamplingRatio, learning_rate)
